...
Plus spécifiquement un camion doit avoir un poids pour sa cargaison et une voiture, le nombre de passagers qu'elle transporte.
...
Un camion coûte pour la traversée un prix de 45 € + 0.1 € * (poids de sa cargaison).
Une voiture coûte (avec son conducteur) 35 € + 3 € * (nombre de passagers).
...
Les tickets sont enregistrés et triées par ordre alphabétique des conducteurs.
...
Nous avons donc choisi de créer un module qui se chargerait de la gestion des véhicules dans l'application et un autre du prix du trajet.
...
Par rapport au sujet, nous avons pris la liberté de modifier la classe véhicule.
...
Nous avons créer une classe position qui nous permet de transmettre plus facilement la rangée et la position dans la rangée.
...
Nous avons fait ce choix pour éviter la duplication d'information même si, une fois enregistré, le véhicule ne peut plus changer.
...
#### Les choix sur l'interface graphique
...
Ce sont les seules libertés que nous ayons prises par rapport au sujet.
...
Elle centralise toutes les opérations de l'application et permet la communication entre la cale du bateau (wedge), la billetterie et les prix (Accounting).
...
Nous avons choisi à lister tous les tickets faits pour les véhicules dans le bateau directement mais nous aurions pu les mettre dans la cale, c'est un choix objectif qui a été fait.
...
On implémente le set avec un treeSet car on ne veut pas d'éléments nuls et il est plus simple de manipuler les éléments à l'intérieur.
...
Elle enregistre les prix de chaque élément et on peut connaitre le prix de l'élément en appelant la méthode getPrice.
...
Nou traitera ces cas par l'affichage et seulement pour un bateau à deux rangées.
...
Et grâce au Set, elle va les organiser selon leur méthode compareTo qu'on détaillera un peu après.
...
On peut voir comment est implémenté la méthode compareTo de "ROW" pour vérifier que c'est le poids de chacune des rangées qui est comparé.

![Méthode commpareTo d'une rangée](./img/compare_to_row.png)

Sur cette capture d'écran on a la fonction compareTo de la classe "ROW" qui compare simplement le poids de la rangée avec une autre passé en paramètre.
Le poids de la rangée est le poidsde tous les véhicules qui s'y trouvent.

#### Mise à jour des éléments de la fenêtre de la cale

![Mise à jour des informations de la fenêtre wedge](./img/update_wedge_window.png)

Dans ce bout de code on peut voir une fonction update.
Elle permet de mettre à jour la fenêtre montrant la cale du bateau.
La fonction setlistdata de leftrowlist et rightrowlist (JList) permet de remplacer le contenu des JList par les arrays d'objet passé en paramètres.
Ce rôle est rempli par listdatavehicle qui va récupérer les informations de rangées et retourner un array d'objet.
Donc à chaque fois qu'on effectue une opération d'ajour ou de suppression de véhicule, on appelle cette fonction pour mettre a jour la cale du bateau

#### Formatage des nombres dans le formulaire d'embarquement

![Formatage des nombres](./img/Formattage_des_nombres.png)
...
Par exemple, on ne veut pas que le nombre de passagers soit à virgule.
...
Le travail a été partagés équitablement entre les personnes du binôme.
Nous avons essayé de séparer en deux le travail pour la partie graphique et la partie console, mais ce n'étais pas facile de gérer la répartition en fonction des compétences de chacun.
Finalement on a réussi à produire le résultat demandé en équipe.
...
En vérifiant à la main, on a le 3ième camions qui ne peuvent être chargé dans la cale à cause d'un manque de place.
...
![Toutes les fenêtres de l'interface graphique](./img/GUI_full_windows.png)
...
La gestion de la possibilité d'embarquer est gérée par la fenêtre d'embarcation
...
Nous avons déjà chargé 3 véhicules. On voit sur cette image les informations du véhicule de Mme DURAND Marie en sélectionnant le véhicule sur l'interface (nécessite un double clics).
On pourra vérifier que les informations sont justes et que les positions sont correctes. Malgré tout ce n'est que l'affichage du modèle précédemment développé dans la partie console.
...
Les points a noté sont que pour une voiture, le champ du poids de la cargaison est inaccessible.
Nous n'avons pas de champ cargaison pour une voiture donc il ne sert a rien de l'activé.
Si tout se passe bien la fenêtre se ferme sans aucun message. Sinon une fenêtre indiqueras l'l'erreur qui est survenue.
...
Et pour le bonus nous vérifions si le bateau n'a pas déjà chargé un véhicule avec un même conducteur ou un autre véhicule avec la même plaque d'immatriculation (car censée être unique)
...
On a fait des vérifications succinctes pour cette classe comme c'est des calculs assez basiques.
On fait une vérification pour seulement quelques camions et quelques voitures.

#### Dans le programme graphique

Pour tester la partie graphique, nous n'avons pas créé de tests automatiques.
...
Le travail en binôme a été réaliser en séparant la charge de travail dans le binôme. Nous avons chacun choisissent les parties de l'application qui nous intéressait le plus à développer et avons partagé et mis en commun notre travail facilement grâce à Github.
...
Il n'y a pas mal d'améliorations que l'on pourrait apporter a ce projet mais par manque de temps nous avons donc on a fait le choix de ne pas les implémenter.
...
Dans la vraie vie, un bateau a une ligne de flottaison faut respecter donc une amélioration la disposition des véhicules est a considéré dans le cas d'un bateau qui contient plus de deux rangées.
...
Mais cela est pour une gestion beaucoup plus approfondie du projet et n'est pas forcément intéressant dans le cadre d'apprentissages de l'algorithmique de base.
